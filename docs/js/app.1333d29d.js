    ...
    "NHnr": function (t, s, e) {
        Object.defineProperty(s, "__esModule", { value: !0 }); let i=e("/5sW"),
            ...
            u={
                ...
                created() { r.a.get("user_durations")||this.$store.dispatch("USER_DURATIONS_SET", this.durations); },
            ...
            d=[],
            l=e("XyMi"); function m(t) { e("69In"); } let p=!1,
            h=m,
            E="data-v-3f02c3ff",
            T=null,
            f=Object(l.a)(u, c, d, p, h, E, T),
            v=f.exports,
            g={
            ...
            I=[]; function S(t) { e("Sq7x"); } let D=!1,
            M=S,
            w="data-v-6ffd3c4c",
            A=null,
            U=Object(l.a)(g, R, I, D, M, w, A),
            C=U.exports,
            O={
                computed: Object.assign({}, Object(n.b)({ modal: "appModal", toast: "toast" })), components: { Timer: v, Settings: C }, data() { return {}; }, methods: { toggleSettings() { this.$store.dispatch("MODAL_TOGGLE"); } }, created() { this.$store.dispatch("APP_INIT"); },
            },
            b=function () {
                let t=this,
                    s=t.$createElement,
            ...
            }]; function N(t) { e("AhgF"); } let L=!1,
            y=N,
            x="data-v-e9dad536",
            G=null,
            $=Object(l.a)(O, b, P, L, y, x, G),
            k=$.exports,
            H=[{ name: "timer", path: "/", component: v }],
            j=new a.a({ mode: "history", routes: H }),
            z=j,
            B={
                ...
                durations: {
                    focus: {
                        title: "focus", duration: 0, minutes: 0, seconds: 0, defaults: { minutes: 0, seconds: 8 },
                    },
                    short_break: {
                        title: "short break", duration: 0, minutes: 0, seconds: 0, defaults: { minutes: 0, seconds: 3 },
                    },
                    long_break: {
                        title: "long break", duration: 0, minutes: 0, seconds: 0, defaults: { minutes: 0, seconds: 8 },
                    },
                },
                schedule: [],
            },
            ...
            J={
                ...
                TIMER_INIT(t) {
                    let s=t.commit,
                        e=_.first(tt.state.schedule),
                        i=e.duration; s("TIMER_UPDATE", { current: e }), s("TIMER_INIT"); let n=parseInt(tt.state.timer.time/60, 10),
                        a=parseInt(tt.state.timer.time%60, 10); n=n<10?`0${n}`:n, a=a<10?`0${a}`:a, s("TIMER_UPDATE", { time: Math.round(i), duration: Math.round(i), output: "".concat(n, ":").concat(a) }), s("TIMER_UPDATE_STYLE");
                },
                TIMER_START(t) { const s=t.commit; s("TIMER_UPDATE", { state: "playing" }), tt.dispatch("TIMER_STEP"), s("TIMER_CLEAR"), tt.state.timer.timer=setInterval(() => { tt.dispatch("TIMER_STEP"); }, 1e3); },
                TIMER_STOP(t) { const s=t.commit; s("TIMER_UPDATE", { state: "paused" }), s("TIMER_CLEAR"); },
                TIMER_RESET(t) { const s=t.commit; s("TIMER_UPDATE", { state: "stopped" }), s("TIMER_CLEAR"), tt.dispatch("SCHEDULE_CREATE"); const e=_.first(tt.state.schedule); tt.dispatch("TIMER_INIT", e.duration); },
                TIMER_UPDATE(t, s) { const e=t.commit; e("TIMER_UPDATE", s); },
                TIMER_STEP(t) {
                    let s=t.commit,
                        e=parseInt(tt.state.timer.time/60, 10),
                        i=parseInt(tt.state.timer.time%60, 10); e=e<10?`0${e}`:e, i=i<10?`0${i}`:i, tt.state.timer.time>=0?(s("TIMER_UPDATE", K()({ output: `${e}:${i}`, percent: 100-tt.state.timer.time/tt.state.timer.duration*100 }, "output", `${e}:${i}`)), s("TIMER_UPDATE_STYLE"), s("TIMER_UPDATE", { time: tt.state.timer.time-1 }), document.title="(".concat(tt.state.timer.output, ") - ").concat(tt.state.timer.current.title)):tt.state.timer.time<=0&&tt.dispatch("TIMER_NEXT");
                },
                TIMER_NEXT(t) { const s=t.commit; tt.dispatch("PLAY_SOUND"), s("TIMER_CLEAR"), s("SCHEDULE_PULL"); const e=_.first(tt.state.schedule); if (e)s("TIMER_UPDATE", { current: e }), tt.dispatch("TIMER_INIT", e.duration), tt.dispatch("TIMER_START"), s("TIMER_UPDATE", { state: "playing" }); else { s("SCHEDULE_CREATE"); const i=_.first(tt.state.schedule); tt.dispatch("TIMER_INIT", i.duration), tt.dispatch("TIMER_START"), s("TIMER_UPDATE", { current: i, state: "playing" }); } },
                ...
                MODAL_TOGGLE(t) { const s=t.commit; tt.dispatch("TIMER_STOP"), s("MODAL_TOGGLE"); },
            },
            V=J,
            W=(e("VjuZ"), {
                APP_INIT(t) { _.each(t.durations, (t, s) => { t.minutes=t.defaults.minutes, t.seconds=t.defaults.seconds; }); }, TIMER_INIT(t) { t.timer.duration=0, t.timer.time="", t.timer.output="00:00", t.timer.percent=0, t.timer.state="stopped", t.timer.timer=""; }, TIMER_UPDATE(t, s) { _.each(s, (s, e) => { t.timer[e]=s; }); }, TIMER_CLEAR(t, s) { window.clearInterval(t.timer.timer), document.title="pomodoro"; }, TIMER_UPDATE_STYLE(t) { t.progressStyle.pie=t.timer.percent>50?"clip: rect(auto, auto, auto, auto);":"", t.progressStyle.left=`transform: rotate(${3.6*t.timer.percent}deg);`, t.progressStyle.right=t.timer.percent<50?"display: none;":"transform: rotate(180deg);"; }, SCHEDULE_CREATE(t) { t.schedule=[t.durations.focus, t.durations.short_break, t.durations.focus, t.durations.short_break, t.durations.focus, t.durations.long_break]; }, SCHEDULE_PULL(t) { _.pullAt(t.schedule, 0); }, USER_DURATIONS_GET(t) { const s=r.a.getJSON("user_durations"); _.each(s, (s, e) => { t.durations[e].title=s.title, t.durations[e].duration=s.duration; }); }, USER_DURATIONS_SET(t, s) { const e={}; _.each(s, (s, i) => { e[i]=s, t.durations[i].title=s.title, t.durations[i].duration=s.duration; }), r.a.set("user_durations", e); }, USER_DURATIONS_RESET(t) { const s={}; _.each(t.durations, (e, i) => { s[i]=e, s[i].title=i.replace("_", " "), t.durations[i].title=i.replace("_", " "), t.durations[i].duration=60*t.durations[i].defaults.minutes+t.durations[i].defaults.seconds; }), r.a.set("user_durations", s); }, USER_PREFERENCES_SET(t, s) { const e={}; _.each(s, (t, s) => { e[s]=t.value; }), r.a.set("user_preferences", e); }, MODAL_TOGGLE(t) { t.app.modal=!t.app.modal; }, TOAST_TRIGGER(t, s) { t.app.toast.message=s, t.app.toast.active=!0, setTimeout(() => { t.app.toast.active=!1; }, 3e3); },
            }),
            Z=W; i.a.use(n.a); var Q=new n.a.Store({
                state: F, getters: X, actions: V, mutations: Z,
            }),
            tt=Q; i.a.use(n.a), i.a.use(a.a), window._=e("M4fF"), window.debug_mode=!0, window.log=function (t) { window.debug_mode&&console.log(t); }; new i.a({
            store: tt, router: z, el: "#app", components: { App: k }, methods: {}, render(t) { return t(k); },
        });
    },
    "Sn+e": function (t, s, e) { t.exports=`${e.p}media/glass-bell.863559b1.mp3`; },
    "Sq7x": function (t, s) {},
}, [0]);
// # sourceMappingURL=app.1333d29d.js.map
